...
async function run() {
  ...
  Deno.writeTextFileSync(
    "iwara_videos_all.json",
    JSON.stringify(allIwara, null, 2),
  );
  Deno.writeTextFileSync(
    "iwara_videos_filtered.json",
    JSON.stringify(filteredIwara, null, 2),
  );

  logger.info(
    "Exported to iwara_videos_all.json and iwara_videos_filtered.json",
  );
}
