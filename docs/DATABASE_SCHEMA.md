...
## 5. `BotSubmission` (bot_submissions)
...
> web UI must not erase the record that the bot was once asked for it — the
> submission survives with `canonicalUrl = NULL`.

> [!NOTE]
> `downloadedByBot` is what makes "never delete a file the bot did not download"
> enforceable in SQL rather than by convention. See
